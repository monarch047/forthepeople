// ═══════════════════════════════════════════════════════════
// ForThePeople.in — Nagaon District Data Seed
// © 2026 Jayanth M B. MIT License with Attribution.
// https://github.com/jayanthmb14/forthepeople
//
// Run: npx tsx prisma/seed-nagaon-data.ts
// ═══════════════════════════════════════════════════════════
...
async function main() {
  console.log("🌱 Seeding Nagaon district data...\n");
  ...
  const district = await prisma.district.findFirst({
    where: { stateId: state.id, slug: "nagaon" },
  });
  if (!district) throw new Error("Nagaon district not found — run seed-hierarchy.ts first");

  const districtId = district.id;
  console.log(`✓ Found Nagaon district (id: ${districtId})`);

  // Clear and re-seed Nagaon data in a transaction
  await prisma.$transaction(async (tx) => {
    // Clear existing Nagaon specific entries
    ...
    console.log("✓ Cleared old entries for Nagaon");
    ...
    await tx.localIndustry.createMany({
      data: [
        ...
        },
        {
          ...
          source: "Horticulture Department Nagaon"
        },
        {
          districtId,
  ...
  console.log("\n🎉 Nagaon district seeding complete!");
}
