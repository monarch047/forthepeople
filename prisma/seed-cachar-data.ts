// ═══════════════════════════════════════════════════════════
// ForThePeople.in — Cachar District Data Seed
// © 2026 Jayanth M B. MIT License with Attribution.
// https://github.com/jayanthmb14/forthepeople
//
// Run: npx tsx prisma/seed-cachar-data.ts
// ═══════════════════════════════════════════════════════════
...
async function main() {
  console.log("🌱 Seeding Cachar district data...\n");
  ...
  const district = await prisma.district.findFirst({
    where: { stateId: state.id, slug: "cachar" },
  });
  if (!district) throw new Error("Cachar district not found — run seed-hierarchy.ts first");

  const districtId = district.id;
  console.log(`✓ Found Cachar district (id: ${districtId})`);

  // Clear and re-seed Cachar data in a transaction
  await prisma.$transaction(async (tx) => {
    // Clear existing Cachar specific entries
    ...
    console.log("✓ Cleared old entries for Cachar");
    ...
    await tx.localIndustry.createMany({
      data: [
        ...
        },
        {
          ...
          source: "Horticulture Department Cachar"
        },
        {
          districtId,
  ...
  console.log("\n🎉 Cachar district seeding complete!");
}
