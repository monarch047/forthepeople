// ═══════════════════════════════════════════════════════════
// ForThePeople.in — Sonitpur District Data Seed
// © 2026 Jayanth M B. MIT License with Attribution.
// https://github.com/jayanthmb14/forthepeople
//
// Run: npx tsx prisma/seed-sonitpur-data.ts
// ═══════════════════════════════════════════════════════════
...
async function main() {
  console.log("🌱 Seeding Sonitpur district data...\n");
  ...
  const district = await prisma.district.findFirst({
    where: { stateId: state.id, slug: "sonitpur" },
  });
  if (!district) throw new Error("Sonitpur district not found — run seed-hierarchy.ts first");

  const districtId = district.id;
  console.log(`✓ Found Sonitpur district (id: ${districtId})`);

  // Clear and re-seed Sonitpur data in a transaction
  await prisma.$transaction(async (tx) => {
    // Clear existing Sonitpur specific entries
    ...
    console.log("✓ Cleared old entries for Sonitpur");
    ...
    await tx.localIndustry.createMany({
      data: [
        ...
        },
        {
          ...
          source: "Horticulture Department Sonitpur"
        },
        {
          districtId,
  ...
  console.log("\n🎉 Sonitpur district seeding complete!");
}
