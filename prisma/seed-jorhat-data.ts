// ═══════════════════════════════════════════════════════════
// ForThePeople.in — Jorhat District Data Seed
// © 2026 Jayanth M B. MIT License with Attribution.
// https://github.com/jayanthmb14/forthepeople
//
// Run: npx tsx prisma/seed-jorhat-data.ts
// ═══════════════════════════════════════════════════════════
...
async function main() {
  console.log("🌱 Seeding Jorhat district data...\n");
  ...
  const district = await prisma.district.findFirst({
    where: { stateId: state.id, slug: "jorhat" },
  });
  if (!district) throw new Error("Jorhat district not found — run seed-hierarchy.ts first");

  const districtId = district.id;
  console.log(`✓ Found Jorhat district (id: ${districtId})`);

  // Clear and re-seed Jorhat data in a transaction
  await prisma.$transaction(async (tx) => {
    // Clear existing Jorhat specific entries
    ...
    console.log("✓ Cleared old entries for Jorhat");
    ...
    await tx.localIndustry.createMany({
      data: [
        ...
        },
        {
          ...
          source: "Horticulture Department Jorhat"
        },
        {
          districtId,
  ...
  console.log("\n🎉 Jorhat district seeding complete!");
}
