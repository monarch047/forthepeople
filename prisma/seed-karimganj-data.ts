// ═══════════════════════════════════════════════════════════
// ForThePeople.in — Karimganj District Data Seed
// © 2026 Jayanth M B. MIT License with Attribution.
// https://github.com/jayanthmb14/forthepeople
//
// Run: npx tsx prisma/seed-karimganj-data.ts
// ═══════════════════════════════════════════════════════════
...
async function main() {
  console.log("🌱 Seeding Karimganj district data...\n");
  ...
  const district = await prisma.district.findFirst({
    where: { stateId: state.id, slug: "karimganj" },
  });
  if (!district) throw new Error("Karimganj district not found — run seed-hierarchy.ts first");

  const districtId = district.id;
  console.log(`✓ Found Karimganj district (id: ${districtId})`);

  // Clear and re-seed Karimganj data in a transaction
  await prisma.$transaction(async (tx) => {
    // Clear existing Karimganj specific entries
    ...
    console.log("✓ Cleared old entries for Karimganj");
    ...
    await tx.localIndustry.createMany({
      data: [
        ...
        },
        {
          ...
          source: "Horticulture Department Karimganj"
        },
        {
          districtId,
  ...
  console.log("\n🎉 Karimganj district seeding complete!");
}
